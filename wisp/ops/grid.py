...
def hashgrid_naive(coords, resolutions, codebook_bitwidth, lod_idx, codebook):
    ...
    for i, res in enumerate(resolutions[:lod_idx + 1]):
        tf_coords = torch.clip(((coords + 1.0) / 2.0) * res, 0,
                               res - 1 - 1e-5).reshape(-1, 3)
        ...
        coeffs = _C.ops.spc.coords_to_trilinear_cuda(tf_coords.contiguous(),
                                                     cc000.contiguous())[...,
                                                                         None]
...
class HashGridInterpolate(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, coords, resolutions, codebook_bitwidth, lod_idx,
                *codebook):
        # TODO(ttakikawa): Make the kernel use the LOD
        feats_out = wisp_C.ops.hashgrid_interpolate_cuda(
            coords.contiguous().float(), codebook, resolutions,
    ...
    def backward(ctx, grad_output):
        ...
        grads = wisp_C.ops.hashgrid_interpolate_backward_cuda(
            coords.contiguous(), grad_output.contiguous(), resolutions,
        ...
        grad_coords = sum(grad_coords)

        return (grad_coords.sum(1), None, None, None, *grad_codebook)
...
def hashgrid(coords, resolutions, codebook_bitwidth, lod_idx, codebook):
    ...
    feats = HashGridInterpolate.apply(
        coords.reshape(-1, dim).contiguous(), resolutions, codebook_bitwidth,
        lod_idx, *[_c for _c in codebook])
